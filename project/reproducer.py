        ...
        for tag in self.round_content:
            ...
            if "<REACH" in tag and 'step="1"' in tag:
                who_called_riichi = self._normalize_position(
                    self.player_position, self.decoder.parse_who_called_riichi(tag)
                )
...
def parse_args_and_start_reproducer():
    ...
    parser.add_option("-l", "--local_log", type="string", help="Path to local log file")
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="Special option for tenhou log reproducer. " "If true, it will print all available tags in the round",
    )
